...
const youtube = new YouTube("AIzaSyDPnwCYgzptm8HqmLNVA0-DbPmJ3HVDo40"); //تعديل اساسي سوي اي بي اي جديد
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyDPnwCYgzptm8HqmLNVA0-DbPmJ3HVDo40"; ///تعديل اساسي سوي اي بي اي جديد
...
client.on("ready", () => {
  ...
  client.user.setActivity(`${prefix}help / https://discord.gg/dfySn9AYHP`);
});
...
\`${prefix}c\` : لمعرفة رصيدك  
\`${prefix}d\` : لأخذ جائزة يومية
...
type these numbers to confirm: `
      ...
      message.channel.send(
        `**:x: | Error , Please Command True Ex: \`${prefix}c [MentionUser] [Balance]\`**`
      );
    }
  }
  if (args[0].toLowerCase() === `${prefix}d`) {
    ...
    if (Daily !== null && cooldown - (Date.now() - Daily) > 0) {
      ...
      let ammount = (2000, 1500, 1000, 900);
